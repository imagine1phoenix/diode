...
export default function AITriageDrawer({ alert, isOpen, onClose }) {
  ...
  const [modelLabel, setModelLabel] = useState('Qwen 3.8 27B');
  const chatEndRef = useRef(null);

  // Fetch active Groq model config
  useEffect(() => {
    fetch(getApiUrl('/api/copilot/config'))
      .then((res) => res.json())
      .then((data) => {
        if (data?.model) {
          const clean = data.model.includes('/') ? data.model.split('/')[1] : data.model;
          setModelLabel(clean.toUpperCase());
        }
      })
      .catch(() => {});
  }, [isOpen]);
  ...
  const handleSendChat = async (textToSend) => {
    ...
    try {
      ...
      setChatMessages((prev) => [
        ...
        {
          role: 'assistant',
          content: `⚠️ Communication error with Groq inference engine. Ensure FastAPI backend is active.`,
          timestamp: new Date().toLocaleTimeString(),
        },
      ]);
    } finally {
    ...
    >
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '2px', fontWeight: '500' }}>
                Groq {modelLabel} • Ultra-Fast LPU Inference • Unidirectional Telemetry
              </p>
            </div>
          </div>
        ...
        <div style={{
          ...
          {/* Fixed Groq Status Badge */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '3px 10px', borderRadius: '6px', background: 'rgba(16, 185, 129, 0.12)', color: '#10B981', border: '1px solid rgba(16, 185, 129, 0.3)', fontSize: '11px', fontWeight: '700', fontFamily: 'var(--font-mono)' }}>
            <Zap size={11} color="#10B981" />
            <span>GROQ: {modelLabel}</span>
          </div>
        </div>
          ...
          <div style={{
            ...
            {loading ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: '14px', padding: '60px 0', color: 'var(--text-secondary)' }}>
                ...
                <div style={{ fontSize: '13px', fontWeight: '600' }}>Running Groq {modelLabel} telemetry evaluation...</div>
              </div>
            ) : triageData ? (
              <>
            ...
            <div style={{
                ...
                return (
                  ...
                  >
                    <div style={{
                      ...
                      <span>{isUser ? 'ANALYST' : (msg.model ? msg.model.toUpperCase() : 'GROQ (LLAMA 3.3 70B)')}</span>
                      <span>•</span>
              ...
              {chatLoading && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-secondary)', fontSize: '12px', padding: '10px' }}>
                  ...
                  <span>Groq {modelLabel} evaluating telemetry...</span>
                </div>
              )}
              <div ref={chatEndRef} />
            </div>
            ...
            <div style={{
              ...
              >
                <input
                  ...
                  placeholder={`Ask Diode Copilot (Groq ${modelLabel}) anything...`}
